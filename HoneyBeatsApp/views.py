...
def index(request):
	...
	#return render(request, 'HoneyBeatsApp/index.html',context)
	return render(request,'HoneyBeatsApp/index.html');
...
def callback(request):
	return render(request, 'HoneyBeatsApp/spotifySelect.html',{})
...
def changeStyleSheet(request):
	print os.getcwd();
	customStylesheet = "HoneyBeatsApp/static/customSheets/styles_"+str(randint(0,99999))+".css"
	newCss = open(customStylesheet,'w+')
	templateCss = open("HoneyBeatsApp/static/customSheets/stylesheetTemplate.css",'r')
...
def saveStyle(request):
	shutil.copy("HoneyBeatsApp"+ request.POST['filename'], "HoneyBeatsApp/savedSheets/" + request.POST['filename'][-10:])
	return HttpResponse()
